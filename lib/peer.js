...
class Peer {
  constructor (config, candidateHandler, iceStateChangeHandler, trackHandler) {
    ...
    function handleICEConnectionStateChangeEvent (handler, event) {
      console.log(`[ICE] connection state changed to: ${p.iceConnectionState}`)

      handler(p.iceConnectionState)
    }
    ...
    p.oniceconnectionstatechange = handleICEConnectionStateChangeEvent.bind(null, iceStateChangeHandler)
